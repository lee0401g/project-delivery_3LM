...
| 模式 | Open WebUI `v0.11.3` 畫面文字 | 系統如何取得內容 | 適合情況 | 注意事項 |
| --- | --- | --- | --- | --- |
| Focused Retrieval（聚焦檢索） | 使用聚焦檢索（Using Focused Retrieval） | 依問題找出部分相關內容 | 多份文件、較長文件與一般問答 | 可能遺漏沒有被檢索到的段落 |
| Full Context（完整內容） | 使用完整檔案（Using Entire Document） | 將完整文件內容提供給模型（加入上下文） | 單一且較短的文件 | 文件過長時會增加等待時間並排擠對話內容 |
...
> 補充教材使用的三份固定文件位於 [單元 06 參考測試資料夾](參考測試資料)
...
7. 在訊息輸入區輸入 `#`
8. 從清單選擇單元 05 建立的知識庫
9. 知識庫名稱出現在訊息輸入區上方後，點選知識庫名稱
10. 在開啟的視窗中確認顯示「使用聚焦檢索（Using Focused Retrieval）」
11. 確認旁邊的開關位於關閉位置，再關閉視窗並返回訊息輸入區

> 開關位於關閉位置不代表停用檢索，而是沒有改用完整檔案，目前仍使用 Focused Retrieval
...
2. 輸入 `#`，只選擇單元 05 的 `設備借用與歸還規則_v1.md`
3. 點選訊息輸入區上方的文件名稱
4. 確認顯示「使用聚焦檢索（Using Focused Retrieval）」且開關位於關閉位置，再關閉視窗
5. 輸入下列問題並記錄回答內容、來源與等待時間

```text
請整理設備借用的預約、領取、歸還及例外情況
每一項都要附上來源
```
...
7. 輸入 `#` 並選擇相同文件
8. 點選訊息輸入區上方的文件名稱
9. 將開關切換至開啟，確認顯示「使用完整檔案（Using Entire Document／Full Context）」後關閉視窗
10. 輸入完全相同的問題
11. 使用下表比較兩次結果

| 比較項目 | Focused Retrieval | Full Context |
| --- | --- | --- |
| 回答是否涵蓋四個指定主題 |  |  |
| 回答使用的來源 |  |  |
| 是否遺漏跨段落內容 |  |  |
...
> 本次使用單一短文件比較。不要將包含多份長文件的整個知識庫切換為 Full Context

## 5 成功檢查

- 新對話已選擇 `phi4-mini:3.8b-q4_K_M` 與指定知識庫
- 第一次測試顯示「使用聚焦檢索（Using Focused Retrieval）」且模式開關位於關閉位置
- 回答包含可開啟的來源標記
- 至少三項回答主張已與原始文件比對
- 能判定主張為直接支持、部分支持或不支持
- 資料不足測試沒有出現無來源的肯定答案
...
知識庫或文件出現在訊息輸入區上方後，還要再點選該項目，才會開啟包含模式名稱與開關的視窗

若畫面顯示「使用聚焦檢索（Using Focused Retrieval）」且開關位於關閉位置，代表目前使用聚焦檢索，不需要將開關打開；開啟後會改用「使用完整檔案（Using Entire Document）」
...
文件內容可能超出模型可處理的範圍，點選已連結的項目，將模式切換回「使用聚焦檢索（Using Focused Retrieval）」或改用更短的單一文件
