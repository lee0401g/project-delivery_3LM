...
地端 AI 是在個人或組織**自行擁有、管理或控制的運算設備上執行**模型，而非將內容交由外部雲端模型服務處理
...
- 判斷地端 AI 時，應確認模型執行位置、設備管理權與資料傳送範圍
- 瀏覽器只是操作介面，不能單憑使用瀏覽器判斷模型位於雲端
- 使用者、Open WebUI、Ollama 與本機模型各自負責不同工作
- 對話流程包含問題傳送、模型執行與回答回傳
...
- 上一單元：[單元 00 課前硬體與環境檢查](../00-課前硬體與環境檢查/README.md)
- 下一單元：[單元 02 Docker Desktop 與 WSL2 安裝](../02-Docker-Desktop-與-WSL2-安裝/README.md)
