...
> 本單元先完成知識庫與文件索引，單元 06 再進行文件問答與引用查核
...
> 本單元使用知識庫集中儲存同一主題的文件，單元 06 再使用這些文件進行問答與引用查核
...
1. 在本機建立一個本次實作專用資料夾
2. 從 [單元 05 參考測試資料夾](參考測試資料) 複製 `設備借用與歸還規則_v1.md` 至實作資料夾
3. 再選擇一至四份可信且來源已知的個人文件，合計準備兩至五份文件
4. 確認本機保留 `設備借用與歸還規則_v1.md`，供單元 07 進行文件更新實作
5. 確認每個檔名都能辨識內容與版本
