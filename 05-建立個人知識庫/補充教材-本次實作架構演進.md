...
本文件供想了解單元 04～05 架構變化的學生與教師查閱，說明 Knowledge（知識庫）加入前後的元件、資料與問答流程

## 建立知識庫前

單元 04 只有一般模型對話，使用者的問題經由 Open WebUI 傳送至 Ollama，再由 `phi4-mini:3.8b-q4_K_M` 產生回答
...
sequenceDiagram
    ...
    participant C as phi4-mini:3.8b-q4_K_M
    U->>B: 輸入問題
    B->>W: 傳送問題
    W->>O: 要求執行對話模型
    O->>C: 載入問題
...
單元 05 沒有增加新容器，而是在原有的 Ollama 與 Open WebUI 中加入新的工作
...
flowchart TD
    ...
    O --> C[phi4-mini:3.8b-q4_K_M<br/>對話模型]
...
| 模型角色 | `phi4-mini:3.8b-q4_K_M` 負責回答 | `phi4-mini:3.8b-q4_K_M` 仍負責回答，同時增加 `embeddinggemma:300m-qat-q4_0` 負責索引與檢索 |
...
sequenceDiagram
    ...
    participant E as embeddinggemma 索引模型
    participant I as 檢索索引
    participant C as phi4-mini:3.8b-q4_K_M 對話模型
    U->>B: 輸入問題並選用知識庫
    B->>W: 傳送問題
    W->>O: 要求處理問題
    O->>E: 轉換問題
...
| Docker volume | 儲存內容 |
| --- | --- |
| `ollama-data` | `phi4-mini:3.8b-q4_K_M`、`embeddinggemma:300m-qat-q4_0` 與 Ollama 資料 |
