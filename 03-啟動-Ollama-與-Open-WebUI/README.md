...
- `README.md`：這份操作說明
- [補充教材-啟動問題排查](補充教材-啟動問題排查.md)：供協助排除問題時查閱
...
使用相同的 Windows 帳號與瀏覽器開啟 Open WebUI 時，瀏覽器通常會保留登入狀態，因此後續可能直接進入聊天介面

> 登出帳號、使用另一個瀏覽器或瀏覽器設定檔、使用無痕模式、清除網站資料，或登入狀態失效後，仍可能再次看到登入頁

> 若使用共用電腦，**使用完畢後應從 Open WebUI 登出**，不要以刪除 Docker 資料的方式要求重新登入
...
> 協助排除問題的人，可參閱 [補充教材-啟動問題排查](補充教材-啟動問題排查.md)
