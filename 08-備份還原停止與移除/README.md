...
重要備份可再複製到另一個磁碟或隨身儲存裝置，避免電腦磁碟故障時原始資料與備份同時遺失
...
- 第 09 單元仍需要使用本機 RAG，正式移除應在成果驗收後執行
...
管理程式會讀取相鄰的第 03 單元資料夾，因此不要單獨移動 `08_data-management.cmd`
...
保持教材原有資料夾結構，不要只複製 `08_data-management.cmd`，第 03 與第 08 單元資料夾應位於同一層
...
使用第 03 單元的 `03_service-control.cmd` 啟動服務，重新下載模型後，再以本單元的最新備份還原 Open WebUI 資料
